/*
    ...
    A little Agressive JS markdown preprocessor for emojis.
    With support for single special markdown symbols markdown like '~', '^', etc
*/
...
const advance = (tip, lim) => {
    return tip < lim ? tip + 1 : tip
}

const isAtEnd = (text, tip) => {
    return tip >= text.length
}
...
const getHTMLTag = (text, tagName) => {
    return `<${tagName}>${text}</${tagName}>`
}

const reformatText = (text, textToInsert, iTip, tip) => {
    let beg = iTip >= 1 ? text.slice(0, iTip - 1) : ''

    // Ordinarily we would parse 'tip' but 'tip' is at tail end of ':' and needs to be offset by two spaces to exclude the trailing ':'
    let end = !isAtEnd(text, tip) ? text.slice(tip + 1) : ''
    let full = beg.concat(textToInsert).concat(end)

    // Place 'tip' at exact end of inserted text
    let newTIP = (iTip + textToInsert.length) - 2
...
const parse = (text, emojisPath, terminals, special_marks={'^': 'sup', '~': 'sub'}, emojiNames=null, verbose=false) => {
    ...
    let debug_verbose_info = []

    let iTip

    while (!isAtEnd(text, tip)) {
        if (text[tip] == ":") {
            tip = advance(tip, text.length)
            iTip = tip

            // To avoid wrongly parsing `:-\n text:-`, etc
            // However, we are still carefull to allow '+1' emoji to be detectable
            if (!alphas.includes(text[tip].toLowerCase()) && !['+', '-', '1', '8'].includes(text[tip])) {
                should_abort = true
            }

            // Reset the to allow normal parsing
            should_skip = false

            if (!should_abort) {
                debug_verbose_info.push(`[+] Initiating parse at: ${tip}`)

                while (text[tip] != ":") {
                    emoji_name = emoji_name.concat(text[tip])

                    if (terminals.includes(text[tip]) || isAtEnd(text, tip)) {
                        should_skip = true
                        emoji_name = ''

                        debug_verbose_info.push('[!] - Broken - skipping parse attempt\n')
                        break
                    }
                ...
                if (!should_skip) {
                    ...
                    if (emojiNames.includes(emoji_name)) {
                        ...
                        // Reset 'tip' and 'text' to current formated text
                        tip = formatedObject.tip
                        text = formatedObject.text

                        debug_verbose_info.push(`[-] Current emoji '${emoji_name}' parsed to '${tag}'\n`)

                        // Reset name
                        emoji_name = ''

                        // Increment number of 'emojis' parsed
                        parsed_emoji_count += 1
            ...
            // reset abort after borked parsing
            should_abort = false

        }

        if (Object.keys(special_marks).includes(text[tip])) {
            // Log symbol to parse
            symbol = text[tip]
            ...
            if (!should_abort) {
                ...
                while (text[tip] != symbol) {
                    tagText = tagText.concat(text[tip])
                ...
                if (!should_skip) {
                    let tag = getHTMLTag(tagText, special_marks[symbol])
                    let formatedObject = reformatText(text, tag, iTip, tip)
                    ...
                    debug_verbose_info.push(`[-] Current tag 'sup' parsed to '${tag}'\n`)
    ...
    debug_verbose_info.push(`[+] Parsed: ${parsed_emoji_count} emojis and ${parsed_syms_count} special symbol(s)`)

    // Print debugging info if 'verbose' flag set
    if (verbose) {
        debug_verbose_info.forEach((msg) => {
            console.log(msg);
        })
    }
